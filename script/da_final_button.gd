...
func _ready() -> void:
	visible = false
	cost.text = "Cost: " + root_node.format_number(DataManager.button_property["DaFinal"]["cost"])
...
func unlock_final() -> void:
	...
	var appear_treshold: int = DataManager.button_property["DaFinal"]["unlock_cost"]
	if DataManager.current_money >= appear_treshold:
		visible = true
		
	#tempo a mettre dans datamanager
	var final_cost: int = DataManager.button_property["DaFinal"]["cost"]
